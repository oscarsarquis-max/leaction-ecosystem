...
describe("receitas", () => {
  ...
  it("abre o atelier com componentes, percentual, processo e assistente", async () => {
    ...
    expect(body).toContain("Farinha de trigo tipo 1 (Demo)");
    expect(body).toContain("Água (Demo)");
    expect(body).toContain("1.000 g");
    expect(body).toContain("650 g");
    expect(body).toContain("100%");
    expect(body).toContain("65%");
    expect(body).not.toContain("1000.000000");
    expect(screen.getByRole("heading", { name: "Percentual do padeiro" })).toBeInTheDocument();
    expect(body).toContain("Total de farinha: 1.000 g");
    expect(screen.getByRole("heading", { name: "Processo" })).toBeInTheDocument();
    expect(body).toContain("Mistura");
    ...
    expect(body).toContain("Fator de escala: 1,9585");
    expect(body).toContain("Massa-base da receita: 1.685 g");
    expect(screen.getByRole("heading", { name: "Ensaios" })).toBeInTheDocument();
    expect(body).toContain("TR-PAO-OK");
    expect(body).toContain("Ensaio concluído");
    expect(body.toLowerCase()).not.toContain("trial concluído");
    expect(screen.getByRole("heading", { name: "Nutrição técnica" })).toBeInTheDocument();
    expect(body).toContain("Prévia técnica incompleta e não validada regulatoriamente");
    ...
    expect(body).toContain("Nutrição incompleta");
    const results = await axe(view.container);
    expect(results.violations).toEqual([]);
  });
