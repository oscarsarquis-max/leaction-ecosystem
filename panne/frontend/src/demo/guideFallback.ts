...
export const DEMO_GUIDE_FALLBACK: DemoGuidePayload = {
  ...
  roadmap: [
    ...
    { step: 8, title: "Consultar receitas", path: "/receitas", requires_session: true },
  ...
  safe_actions: {
    ...
    shared_notice:
      "Antes de uma alteração relevante, lembre: este ambiente é compartilhado. A funcionalidade permanece disponível; a indicação é proporcional.",
  },
  integrations: [
    {
...
export function formatGuideCount(value: number | null | undefined): string {
  if (value == null) return "Não informado";
  return String(value);
}

export function integrationStateLabel(state: string): string {
  switch (state) {
    case "ativo":
      return "Ativo";
    case "simulado":
      return "Simulado";
    case "preparado_desativado":
      return "Preparado / desativado";
    case "indisponivel":
      return "Indisponível neste recorte";
    default:
      return state;
  }
}
