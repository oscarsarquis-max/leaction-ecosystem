...
class InventoryLot(Base):
    ...
    package_content_quantity: Mapped[Decimal | None] = mapped_column(Numeric(18, 6))
    package_content_unit: Mapped[str | None] = mapped_column(Text)
    package_content_declared_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    package_content_declared_by: Mapped[UUID | None] = mapped_column(Uuid)
    cost_status: Mapped[str] = mapped_column(Text, nullable=False, server_default="known")
    declared_unit_cost: Mapped[Decimal | None] = mapped_column(Numeric(18, 6))
    declared_cost_currency: Mapped[str | None] = mapped_column(Text)
    opening_origin: Mapped[str | None] = mapped_column(Text)
