...
app.use((req, res, next) => {
    ...
    res.locals.whatsappCommunityUrl =
        process.env.WHATSAPP_COMMUNITY_URL || 'https://chat.whatsapp.com/placeholder-paneldx-comunidade';
    next();
});
...
app.get('/', async (req, res) => {
    ...
    res.render('index', {
        title: 'Transformação Digital Educacional | Diagnóstico Gratuito Panel DX',
...
// Matriz canônica global OKR (baseline PanelDX)
...
function calcularGamificacaoOkr(direcionadores) {
    ...
    if (fixosComObj >= 3) badges.push({ icon: '⚡', label: 'Momentum', desc: 'Três pilares PanelDX com objetivos vinculados.' });
