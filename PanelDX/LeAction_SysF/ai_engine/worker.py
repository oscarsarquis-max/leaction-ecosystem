...
# --- FOCO NO SERVIÇO AWS ---
# Verificamos se o HOST já existe no ambiente (injetado pela Task Definition do ECS)
# Se ele NÃO existir, carregamos o .env (comportamento para seu ambiente LOCAL)
if not os.getenv("DB_HOST"):
    ...
    if env_path.exists():
        ...
        load_dotenv()
# Se o DB_HOST já existe, NÃO rodamos load_dotenv para evitar que um arquivo interno
# sobrescreva a configuração correta da nuvem com "127.0.0.1".
...
from ai_engine.worker_lock import MASTER_WORKER_PORT, acquire_worker_lock
from ai_engine.gerar_diagnostico_ia import LeActionAIProcessor, KANBAN_ENGINE_VERSION


def main():
    _lock = acquire_worker_lock("Worker IA Master", MASTER_WORKER_PORT)

    print("\n[START] Iniciando script worker.py...", flush=True)
    print(f"[WORKER] Kanban engine: {KANBAN_ENGINE_VERSION}", flush=True)
    host_atual = os.getenv("DB_HOST")
    print(f"DEBUG: Host detectado pelo sistema: {host_atual}", flush=True)

    # Captura as variáveis com fallback para o que está na Task Definition
    # db_name = os.getenv("DB_NAME", "LeAction_SysF")
    # db_user = os.getenv("DB_USERNAME") or os.getenv("DB_USER")  # Aceita os dois padrões
    # db_pass = os.getenv("DB_PASSWORD") or os.getenv("DB_PASS")  # Aceita os dois padrões
    # db_host = os.getenv("DB_HOST")
    # db_port = os.getenv("DB_PORT", "5432")
    # Print de conferência imediata no CloudWatch
                ...
                if row:
                    ...
                    if processor.processar_diagnostico(conn, id_matu):
                        ...
                        if id_ctdi_res:
                            ...
                            # B. ATIVA ONDA 1 (metadado de iteração — status das sprints já definido na Gênese)
                            cur.execute("""
                                        UPDATE ctdi_itera
                                        SET stat_itera = 'em_andamento'
                                        WHERE id_ctdi = %s
                                          AND id_itera =
                                              (SELECT id_itera FROM ctdi_itera WHERE id_ctdi = %s ORDER BY id_itera LIMIT 1)
                                        """, (id_ctdi, id_ctdi))

                            # C. Status das sprints definidos em popular_kanban_priorizado (governança v3)
