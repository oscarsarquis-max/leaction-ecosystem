"""
Proxy PLG Tracking — PanelDX (sensor) → Action Hub (provedor CRM).

Não persiste nada no banco do PanelDX. Enriquece IP/UA e encaminha S2S.
Falhas no Hub NÃO devem travar a navegação do usuário (sempre 202/ok local).
"""
...
def _hub_receber_url() -> str:
    ...
    if explicit:
        return explicit
    base = (os.environ.get('ACTION_HUB_API_URL') or os.environ.get('HUB_API_URL') or 'http://127.0.0.1:4001').strip()
...
def tracking_enviar():
    ...
    try:
        ...
        # Hub fora do ar / timeout — não derruba UX
