<#
.SYNOPSIS
  Publica SOMENTE o frontend de homologação (web_homolog) no Lightsail.
  Não altera o container do piloto (web).
#>
param(
...
$dir = Join-Path $env:TEMP ("homolog-web-" + [guid]::NewGuid().ToString("N").Substring(0, 8))
...
Write-Host "== Rebuild web_homolog only =="
$remote = @'
set -euo pipefail
cd /opt/qmind
...
source .env
set +a
sudo docker compose -f docker-compose.homolog.yml build web_homolog
sudo docker compose -f docker-compose.homolog.yml up -d web_homolog
sudo docker compose -f docker-compose.homolog.yml ps web_homolog
echo HOMOLOG_WEB_OK
'@

$remoteFile = Join-Path $dir "remote.sh"
Write-Lf $remoteFile $remote
scp @sshOpts $remoteFile "${sshTarget}:/tmp/publish-homolog-web.sh"
ssh @sshOpts $sshTarget "bash /tmp/publish-homolog-web.sh; rm -f /tmp/publish-homolog-web.sh"

Write-Host "== Done (homolog only) =="
