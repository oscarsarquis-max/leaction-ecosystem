...
Somente a Spider deve chamar `POST /v1/illustrative-protection-items` (jornada ilustrativa, Provider 1.0) e `POST /v1/provider/capabilities/{capabilityId}/executions` (incluindo `GENERATE_SYNTHETIC_HOME_QUOTE`, Provider 1.1, e `DISCOVER_SYNTHETIC_CROP_PROTECTION_PATHS`, caminhos agrícolas demonstrativos **sem R$**). O SegSense **não** chama este processo.
