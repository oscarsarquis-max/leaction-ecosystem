...
function sanitizeCapture(body) {
  ...
  return {
    ...
    excerptLooksLikeChrome: /entrar|criar uma conta|menu principal|sumário|indice/i.test(excerpt),
    textLength: text.length,
    containsCropFailure: /quebra de safra/i.test(`${excerpt} ${text} ${body.title ?? ''}`),
    extractedKeys: parseElements(body.extractedElementsJson).map((item) => `${item.key}=${item.value}`),
    hasCropMilho: parseElements(body.extractedElementsJson).some(
      (item) => item.key === 'crop' && /milho/i.test(item.value || ''),
    ),
    hasRegionRioGrandeDoSul: parseElements(body.extractedElementsJson).some(
      (item) => item.key === 'region' && /rio grande do sul/i.test(item.value || ''),
    ),
    technical: {
      ...
      selectionStrategy: body.technical?.selectionStrategy ?? null,
    },
  }
}
...
function sanitizeJourney(body) {
  ...
  return {
    ...
    pendingForBroker: Array.isArray(body.pendingForBroker) ? body.pendingForBroker : [],
    urlExtractedElements: urlExtractedElements(body),
  }
}
...
function urlExtractedElements(body) {
  const contribs = Array.isArray(body?.contextContributions) ? body.contextContributions : []
  const url = contribs.find((item) => item && (item.sourceType === 'URL_EXTRACTED' || item.role === 'URL_EXTRACTED'))
  return url?.elements ?? null
}
...
if (crop.json?.publicStatus === 'AWAITING_REVIEW' && crop.json?.captureId) {
  ...
  assert(crop.json.technical?.extractorVersion === 'URL_EXTRACTOR_V2', `extractor ${crop.json.technical?.extractorVersion}`)
  assert(!/entrar|criar uma conta/i.test(crop.json.excerpt || ''), 'main excerpt still starts with Wikipedia chrome')
  const extracted = parseElements(crop.json.extractedElementsJson)
  assert(
    extracted.some((item) => item.key === 'theme' && item.value === 'crop_production_loss'),
    'theme crop_production_loss missing',
  )
  assert(
    !extracted.some((item) => item.key === 'crop' && /milho/i.test(item.value || '')),
    'spurious crop=milho extracted',
  )
  assert(
    !extracted.some((item) => item.key === 'region' && /rio grande do sul/i.test(item.value || '')),
    'spurious region=Rio Grande do Sul extracted',
  )
  ...
  const spiderElements = urlExtractedElements(cropJourney.json) || {}
  assert(spiderElements.crop !== 'milho', `spider received crop=${spiderElements.crop}`)
  assert(
    !/rio grande do sul/i.test(String(spiderElements.region || '')),
    `spider received region=${spiderElements.region}`,
  )
  const pending = cropJourney.json?.pendingForBroker ?? []
  assert(pending.some((item) => /cultura/i.test(item)), 'mock did not ask for crop')
  assert(pending.some((item) => /região/i.test(item)), 'mock did not ask for region')
