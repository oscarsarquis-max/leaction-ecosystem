...
class LeactionCRUD:
    ...
    def _convert_record_to_serializable(self, record):
        """Converte tipos nao serializaveis em JSON (NumericRange, datetime, Decimal, UUID)."""
        if not record:
            return record
        ...
        if isinstance(record, dict):
            ...
            mutable_record = dict(record.__dict__)
        else:
            mutable_record = dict(record)
        ...
        for key, value in mutable_record.items():
            if isinstance(value, NumericRange):
                ...
                mutable_record[key] = value.isoformat()
            elif isinstance(value, Decimal):
                mutable_record[key] = float(value)
    ...
    def _get_pk_column(self, table_name):
        """Metodo auxiliar para obter o nome da coluna da chave primaria de forma robusta."""
        pk_overrides = {
            'ctdi_quest': 'id_ques',
            'ctdi_okr_direcionadores': 'id_direc',
            'ctdi_okr_objetivos_dt': 'id_obj_dt',
            'ctdi_okr_krs': 'id_kr',
            'ctdi_okr_atividades': 'id_ativ',
        }
        return pk_overrides.get(table_name, f"id_{table_name.split('_')[-1].lower()}")
        ...
        try:
            ...
            query = """
                    SELECT s.id_ques, \
                           s.grad_ques, \
                           s.quali_ques, \
                           COALESCE(s.id_dime, q.id_dime) AS id_dime, \
                           COALESCE(s.id_doma, q.id_doma) AS id_doma, \
                           q.prefu_ques
                    FROM ctdi_surv s
    ...
    def read_all_maturities_with_client_name(self, conn, apenas_empresarial=True):
        """
        Lê registros de maturidade com nome/e-mail do cliente (lista Admin).
        Por padrão exclui inovadores solo (init_role SOLO), que não passam
        pelo formulário de solicitação de diagnóstico empresarial.
        """
        # [REMOVIDO] O bloco 'if not self.conn: ...' é removido.
        ...
        try:
            ...
            query = """
                    SELECT t1.*,
                           ...
                           t2.init_role,
                           t2.justificativa_solo
                    FROM ctdi_matu t1
                             JOIN ctdi_clie t2 ON t1.id_clie = t2.id_clie
                    """
            if apenas_empresarial:
                query += """
                    WHERE NOT (
                        COALESCE(UPPER(TRIM(t2.init_role)), 'GENERAL') = 'SOLO'
                        OR COALESCE(UPPER(TRIM(t1.status_ia)), '') = 'SANDBOX'
                        OR NULLIF(TRIM(t2.justificativa_solo), '') IS NOT NULL
                    )
                    """
            query += """
