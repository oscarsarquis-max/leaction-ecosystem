...
def _default_cms_landing():
    return {
        "hero": {
            ...
            "paneldx_title": "MudaEdu",
            "subtitle": "Inovação e agilidade na educação",
...
def _mini_plano_fallback_consultor(amostra_blocos):
    ...
    return (
        ...
        '3) EXECUÇÃO: Agendar diagnóstico completo no MudaEdu para converter o roteiro em tarefas no Kanban.'
    )
...
def gerar_sprint_avulsa():
    ...
    try:
        ...
        metrics_payload = {
            ...
            "schema_versao": "1.0.0",
            "id_clie": id_clie,
            "origem": "Consultor LeAction - MudaEdu",
...
def _resolve_email_sender():
    ...
    for sender in candidates:
        ...
        if any(domain in normalized for domain in SES_PLACEHOLDER_DOMAINS):
            continue
        return sender.strip()
    return 'consultant@mudaedu.com.br'
...
def _resolve_paneldx_public_url():
    ...
    candidates = [
        os.environ.get('MUDAEDU_PUBLIC_URL'),
...
Obrigado por usar o MudaEdu (LeAction System).
        ...
        if request.method == 'PUT':
            ...
            if entity_type == 'direcionadores':
                ...
                if row and row.get('slug_catalogo'):
                    return jsonify({
                        "error": "Direcionadores do catálogo MudaEdu não podem ser removidos."
                    }), 403
...
if __name__ == '__main__':
    ...
    print(f"🚀 MudaEdu Flask iniciando na porta {flask_port}... (reloader={'on' if use_reloader else 'off'})", file=sys.stderr)
