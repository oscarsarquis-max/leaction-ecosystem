...
(function (global) {
    ...
    function enqueueBlocoMesa(block) {
        ...
        var fila = [];
        try {
            var rawFila = sessionStorage.getItem('MudaEdu_mesa_fila_blocos');
            if (rawFila) {
        ...
        fila.push(payload);
        try {
            sessionStorage.setItem('MudaEdu_mesa_fila_blocos', JSON.stringify(fila));
            sessionStorage.setItem('MudaEdu_sprint_prioridade', JSON.stringify(payload));
        } catch (err) {
