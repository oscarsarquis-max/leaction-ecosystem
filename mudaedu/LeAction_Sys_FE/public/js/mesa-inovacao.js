(function () {
    ...
    const FILA_KEY = 'MudaEdu_mesa_fila_blocos';
    const STORAGE_KEY = idMatu ? ('MudaEdu_mesa_org_notas_' + idMatu) : null;
    ...
    function lerFilaBlocos() {
        ...
        try {
            var rawPrior = sessionStorage.getItem('MudaEdu_sprint_prioridade');
            if (rawPrior) {
    ...
    function montarPayloadUniversal() {
        ...
        return {
            metadados: {
                origem: 'Mesa de Inovação - MudaEdu',
                schema_versao: '1.0.0',
    ...
    function montarPayloadTelemetria(nota) {
        ...
        return {
            metadados: {
                origem: 'Telemetria Base Mobile - MudaEdu',
                schema_versao: '1.0.0',
                tipo_mesa: 'organizacional',
                gerado_em: new Date().toISOString(),
                id_matu: idMatu,
    ...
    async function enviarSprintParaKanban(payload, btnRef) {
        ...
        try {
            sessionStorage.setItem('MudaEdu_mesa_org_sprint', JSON.stringify(body));
        } catch (e) {
        ...
        try {
            ...
            if (response.ok && (result.success || result.status === 'success')) {
                sessionStorage.removeItem('MudaEdu_mesa_org_sprint');
                try {
                    sessionStorage.removeItem('MudaEdu_sprint_prioridade');
    ...
    function exportarJson() {
        ...
        if (formato === 'Universal') {
            ...
            alert('Exportação ' + formato + ' disponível em breve. Use Universal para o Kanban MudaEdu.');
            return;
        }
