...
# Garante imports dos módulos copiados junto com este port
...
# Templates locais (cópia fiel do PanelDX inovador_dashboard.ejs)
...
def dashboard_inovador():
    ...
    # 2. Preferir usuário freemium inove4us; senão sessão/URL; senão sandbox 999
    user = session.get('user') or {}
    id_clie = user.get('id_clie') or session.get('id_clie')
    if not id_clie:
        id_clie = 999
...
def logout_inovador_local():
    ...
    session.clear()
    return redirect(os.environ.get('FRONTEND_ORIGIN', 'http://localhost:5174') + '/acesso')
...
def importar_sprint_inovacao():
    ...
    origem_label = (metadados.get('origem') or 'Mesa de Inovação - PanelDX').strip()
    metrics_payload = {
