...
/** Proxy GET /api/crm/contas — injeta x-crm-secret como o funil. */
export async function GET(request: Request) {
  const { searchParams } = new URL(request.url);
  const qs = new URLSearchParams();
  for (const key of ['sistema', 'com_contrato', 'sem_acesso_desde']) {
    const v = (searchParams.get(key) || '').trim();
    if (v) qs.set(key, v);
  }
  const suffix = qs.toString() ? `?${qs.toString()}` : '';
  try {
    const upstream = await fetch(`${gatewayBase()}/api/crm/contas${suffix}`, {
      headers: crmHeaders(),
      cache: 'no-store',
    });
