...
import { listOperationalSites, listOperationalUsers } from '../services/operationalApi';
import {
  assignNonConformity,
  listNonConformities,
  updateNonConformity,
...
import RecurrenceAndCapPanel from './kaizen/RecurrenceAndCapPanel';
...
export default function KaizenBoard() {
  ...
  const [savingNcId, setSavingNcId] = useState(null);
  const [tenantUsers, setTenantUsers] = useState([]);

  const loadBoard = useCallback(async () => {
    setError('');
    try {
      const [response, ncRes] = await Promise.all([
        fetchKaizenKanban({
          operationalSiteId: siteFilter || undefined,
        }),
        listNonConformities({
          operationalSiteId: siteFilter || undefined,
        }).catch(() => ({ non_conformities: [] })),
      ]);
      const ncs = ncRes.non_conformities || [];
      setNonConformities(ncs);
      const ncTicketIds = new Set(
        ncs.map((nc) => nc.source_kaizen_ticket_id).filter(Boolean),
      );
      const raw = { ...emptyKanbanBoard(), ...(response.kanban || {}) };
      const enriched = {};
      for (const stage of KAIZEN_STAGES) {
        enriched[stage.id] = (raw[stage.id] || []).map((ticket) => ({
          ...ticket,
          has_non_conformity: ncTicketIds.has(ticket.id),
        }));
      }
      setBoard(enriched);
    } catch (err) {
  ...
  useEffect(() => {
    listOperationalSites()
      .then((res) => setSites((res.sites || []).filter((s) => s.is_active !== false)))
      .catch(() => setSites([]));
    listOperationalUsers()
      .then((res) => setTenantUsers(res.users || []))
      .catch(() => setTenantUsers([]));
  }, []);

  const ncByTicket = useMemo(() => {
    const map = {};
    for (const nc of nonConformities) {
      if (nc.source_kaizen_ticket_id) map[nc.source_kaizen_ticket_id] = nc;
    }
    return map;
  }, [nonConformities]);

  async function handleNcUpdate(ncId, patch) {
    setSavingNcId(ncId);
    setError('');
    try {
      const res = await updateNonConformity(ncId, patch);
      const updated = res.non_conformity || res;
      setNonConformities((prev) =>
        prev.map((nc) => (nc.id === ncId ? { ...nc, ...updated } : nc)),
      );
    } catch (err) {
      setError(err.message || 'Erro ao atualizar não conformidade.');
    } finally {
      setSavingNcId(null);
    }
  }

  async function handleNcAssign(ncId, patch) {
    setSavingNcId(ncId);
    setError('');
    try {
      const res = await assignNonConformity(ncId, patch);
      const updated = res.non_conformity || res;
      setNonConformities((prev) =>
        prev.map((nc) => (nc.id === ncId ? { ...nc, ...updated } : nc)),
      );
    } catch (err) {
      setError(err.message || 'Erro ao atribuir responsável/prazo.');
    } finally {
      setSavingNcId(null);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex items-center gap-2 text-xs text-slate-600">
            <span className="font-semibold">Canteiro</span>
            <select
          ...
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {nonConformities.map((nc) => (
                <tr key={nc.id}>
                  <td className="px-3 py-3 align-top font-medium text-slate-800">{nc.title}</td>
                  <td className="px-3 py-3 align-top text-xs text-slate-600">
                    {nc.operational_site_name || '—'}
                  </td>
                  <td className="px-3 py-3 align-top text-xs">{nc.category || '—'}</td>
                  <td className="px-3 py-3 align-top text-xs">{nc.severity || '—'}</td>
                  <td className="px-3 py-3 align-top">
                    <select
                      className="rounded border border-slate-200 px-2 py-1 text-xs"
                      value={nc.status}
                      disabled={savingNcId === nc.id}
                      onChange={(e) => handleNcUpdate(nc.id, { status: e.target.value })}
                    >
                  ...
                  <td className="px-3 py-3 align-top">
                    <div className="space-y-0.5">
                      <select
                        ...
                        value={nc.owner_user_id || ''}
                        disabled={savingNcId === nc.id}
                        onChange={(e) =>
                          handleNcAssign(nc.id, {
                  ...
                  <td className="px-3 py-3 align-top">
                    <div className="space-y-0.5">
                      <input
                        ...
                        onChange={(e) =>
                          handleNcAssign(nc.id, {
                            owner_user_id: nc.owner_user_id || null,
                            due_date: e.target.value || null,
                          })
                        }
                      />
                      {nc.owner_source === 'Manual' && (
                        <span className="block text-[10px] text-slate-400">manual</span>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="px-3 py-3 align-top">
                    <input
                      ...
                      onBlur={(e) => {
                        ...
                        if (value !== (nc.corrective_action || '')) {
                          handleNcUpdate(nc.id, { corrective_action: value || null });
                        }
                      }}
      ...
      ) : viewTab === 'recurrence' || viewTab === 'cap' ? (
        <RecurrenceAndCapPanel
          siteFilter={siteFilter}
          subTab={viewTab === 'cap' ? 'projects' : 'signals'}
          onError={setError}
                      ...
                      tickets.map((ticket) => (
                        <KaizenTicketCard
                          ...
                          ticket={{
                            ...ticket,
                            has_non_conformity:
                              ticket.has_non_conformity || Boolean(ncByTicket[ticket.id]),
                          }}
                          stageId={stage.id}
